"""
Script de consolidação de séries históricas do banco Bradesco para execução em AWS Glue.

Este script lê arquivos Excel (.xlsx) armazenados em um bucket S3,
processa as abas relevantes e grava um CSV consolidado de volta no
mesmo bucket utilizando apenas pandas e boto3 (sem Spark). O objetivo
é transformar as planilhas de séries históricas em um formato longo
pronto para consumo.

Principais características:
  - Extrai ``data_divulgacao`` do nome do arquivo (ex.: 2T25 → 2025-06-01).
  - Normaliza ``data_base`` para ``YYYY-MM-01`` e preserva o rótulo
    original em ``data_base_original``.
  - Numera atributos repetidos (#1, #2, …) somente quando o mesmo
    nome aparece em linhas diferentes.
  - Usa pandas para transformar os dados e boto3 para ler e salvar
    objetos em S3. O arquivo de saída é um único CSV escrito via
    ``put_object``.

Para ajustar o script ao seu ambiente, modifique apenas ``base_path``
para apontar para o caminho ``s3://bucket/prefix`` adequado.
"""
...
# ---------------------------------------------------------------------------
# Defina aqui o prefixo base no S3 (formato s3://bucket/prefix)
...
def main():
    ...
    input_prefix = f"{prefix_base}/input/bradesco/series_historicas/"
    execution_date = datetime.now().strftime("%Y-%m-%d")
    # saída sem extensão, Spark cria diretório
    output_prefix = f"{prefix_base}/refined/bradesco/series_historicas/data_ext={execution_date}/bradesco_series"
    sheet_targets = [
    ...
    all_records = []
    for key in keys:
        all_records.extend(process_s3_file(s3_client, bucket, key, sheet_targets, bank_name="bradesco"))
    if not all_records:
    ...
    # Monta a chave de saída com extensão .csv (um único arquivo)
    output_key = f"{output_prefix}.csv"
    # Faz upload diretamente para o S3 usando boto3
