"""
Script de consolidação de séries históricas do banco Santander para execução em AWS Glue.

Lê arquivos Excel (.xlsx) de um bucket S3, extrai as abas indicadas e
gera um único CSV consolidado em formato longo. O resultado é gravado
de volta no S3 usando apenas pandas e boto3 (sem Spark).

Características principais:
  - Extração de ``data_divulgacao`` a partir do nome do arquivo.
  - Normalização de ``data_base`` para ``YYYY-MM-01`` e preservação do
    rótulo original em ``data_base_original``.
  - Numeração de atributos repetidos (#1, #2, …) apenas quando
    aparecem em linhas diferentes.
  - Uso de pandas para manipulação dos dados e boto3 para upload
    direto do CSV para o S3 via ``put_object``.

Defina ``base_path`` para o prefixo S3 apropriado (``s3://bucket/prefix``).
"""
...
def main():
    ...
    input_prefix = f"{prefix_base}/input/santander/series_historicas/"
    execution_date = datetime.now().strftime("%Y-%m-%d")
    output_prefix = f"{prefix_base}/refined/santander/series_historicas/data_ext={execution_date}/santander_series"
    sheet_targets = [
        'Balanço',
        'DMPL',
    ]
    ...
    all_records = []
    for key in keys:
        all_records.extend(process_s3_file(s3_client, bucket, key, sheet_targets, bank_name="santander"))
    if not all_records:
